---
name: ixhlink-skills-tmeet-assistant
version: 1.0.0
description: "腾讯会议 CLI（tmeet）：OAuth 授权登录/登出/状态查询、会议管理（创建/更新/取消/查询/受邀者）、录制管理（列表/下载地址/智能纪要/转写/录制权限申请）、会议报告（参会人/等候室）、会议考勤（应到名单、迟到/早退/缺席、多次进出、周期汇总、CSV/Excel 导出）、通讯录（按用户名/手机号/邮箱搜索成员）、会中控制（呼叫成员入会/踢出会议成员）、问题排查（导出本地日志，反馈工具缺失/失败/能力不足等问题给平台）。当用户需要通过命令行操作腾讯会议、统计单场或周期会议考勤，或 Agent 在使用过程中遇到工具缺失、调用失败、能力不足等情况想反馈给平台时使用本技能。"
metadata:
  requires:
    bins: ["tmeet", "python3"]
  cliHelp: "tmeet --help"
...
基于腾讯会议命令行工具的会议助手，支持 OAuth 授权、会议全生命周期管理、录制与转写、会议报告查询及会议考勤。
...
- **考勤（Attendance）**：基于会议基础信息、完整受邀者名单和完整参会报告进行的本地确定性判定；考勤结论不是腾讯会议原始字段，必须同时保留事实、规则和判定原因。
...
考勤不是 `tmeet` 子命令。它按照 `meeting get` → `meeting invitees-list` → `report participants` 的顺序获取事实，再调用本技能的 `scripts/calculate_attendance.py` 计算和导出。
...
- 会议考勤：[`references/tmeet-attendance.md`](references/tmeet-attendance.md)
- 考勤数据契约：[`references/attendance-schema.md`](references/attendance-schema.md)
...
- **考勤数据不得推断或补造**：考勤前必须拉取受邀者和参会报告的全部分页。分页不完整、缺少入会/离会时间或身份存在多候选时，必须标记数据不足或待确认，不得根据姓名相似度、发言时长、录制画面等信息猜测。会议尚未结束时只能输出临时考勤，禁止提前判定早退。

- **考勤隐私最小化**：考勤仅处理用户指定会议及名单。未经明确要求，不得跨会议追踪个人行为、扩大到部门级人员查询或在问题反馈中提交姓名、手机号、邮箱、会议号、考勤结论等敏感数据。导出文件只包含用户要求的必要字段。

## 响应处理规则

- **只展示关键信息**：在用户没有明确要求的前提下，仅展示与用户问题直接相关的核心字段，不得输出冗余字段。
- **禁止擅自聚合或排序**：未经用户要求，不得对返回结果进行任何聚合统计或排序操作，按原始结果如实呈现。
- **考勤统计例外**：当用户明确要求“考勤”“出勤统计”“迟到早退”“周期会议汇总”等结果时，可按 [`references/tmeet-attendance.md`](references/tmeet-attendance.md) 进行规则驱动的聚合。回复必须同时展示采用的规则、数据完整性警告和判定原因，不得把本地计算结果描述为腾讯会议原始结论。
